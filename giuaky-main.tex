...
\section{3.1. Mô tả tập dữ liệu và cách xây dựng tập huấn luyện}
...
\subsection{3.1.1. Tiêu chí lựa chọn và nguồn dữ liệu}
...
\subsection{3.1.2. Mô tả chi tiết các bộ dữ liệu}
...
\subsection{3.1.3. Quá trình chuẩn bị và gán nhãn dữ liệu}
...
\subsection{3.1.4. Thống kê và phân tích phân bố dữ liệu tổng hợp}
...
\textbf{Bảng 3.1. Phân bố dữ liệu theo nguồn và nhãn cảm xúc.}
...
\textbf{Bảng 3.2. Thống kê độ dài văn bản trong tập dữ liệu (đơn vị: từ)}
...
\subsection{3.1.4. Phân tích đặc thù ngôn ngữ trong dữ liệu}
...
\textbf{Bảng 3.3. Tỷ lệ xuất hiện các hiện tượng ngôn ngữ đặc thù trong tập dữ liệu}
...
\subsection{3.1.5. Quy trình tiền xử lý dữ liệu chi tiết}
...
\subsection{3.1.6. Xây dựng tập huấn luyện, xác thực và kiểm thử}
...
\textbf{Bảng 3.4. Phân bố dữ liệu sau khi chia tập.}
...
\section{3.2. Thiết lập thực nghiệm}
...
\subsection{3.2.1. Môi trường phần cứng và phần mềm}
...
\textbf{Bảng 3.5. Danh sách thư viện và phiên bản sử dụng trong nghiên cứu.}
...
\subsection{3.2.2. Phương pháp biểu diễn đặc trưng}
...
\subsection{3.2.3. Thiết lập siêu tham số và cơ sở lựa chọn}
...
(MultinomialNB) | Alpha (Laplace smoothing): 1.0

Biểu diễn: TF-IDF, max\_features=10000, ngram=(1,2), min\_df=2 |

(linear) | Kernel: linear, C: 1.0

Biểu diễn: TF-IDF, max\_features=10000, ngram=(1,2), min\_df=2 |

min\_samples\_split: 5, criterion: entropy

Biểu diễn: TF-IDF, max\_features=10000, ngram=(1,2), min\_df=2 |

(fine-tune) | Mô hình nền: vinai/phobert-base (135M tham số)

max\_seq\_length: 128, Batch size: 32

Learning rate: 2e-5, Optimizer: AdamW

Weight decay: 0.01, Warmup: 10\%, Epochs: 10

Early stopping patience: 3 |

(lai) | PhoBERT: giống mô hình PhoBERT fine-tune

CNN: 3 nhóm × 128 filters, kernel=[3,4,5], ReLU

LSTM: hidden=256, dropout=0.3

FC: 512→3, dropout=0.5

LR: 1e-5, Epochs: 15, Early stopping: 3 |


\textbf{Bảng 3.6. Tổng hợp siêu tham số của các mô hình.}


\subsection{3.2.4. Chiến lược huấn luyện}
...
\subsection{3.2.5. Phương pháp đánh giá}
...
\section{3.3. Kết quả thực nghiệm}
...
\subsection{3.3.1. Kết quả của các mô hình học máy truyền thống}
...
\textbf{Bảng 3.7. Kết quả đánh giá các mô hình học máy truyền thống trên tập kiểm thử.}
...
\textbf{Bảng 3.8. Kết quả 5-fold cross validation của các mô hình học máy truyền thống.}
...
\subsection{3.3.2. Kết quả của mô hình học sâu PhoBERT}
...
\textbf{Bảng 3.9. Kết quả đánh giá mô hình PhoBERT trên tập kiểm thử.}
...
\textbf{Bảng 3.10. Kết quả PhoBERT theo từng lớp cảm xúc.}
...
\subsection{3.3.3. Kết quả của mô hình lai PhoBERT-CNN-LSTM}
...
\textbf{Bảng 3.11. Kết quả đánh giá mô hình PhoBERT-CNN-LSTM trên tập kiểm thử.}
...
\textbf{Bảng 3.12. Kết quả PhoBERT-CNN-LSTM theo từng lớp cảm xúc.}
...
\textbf{Bảng 3.13. Kết quả ablation study trên kiến trúc mô hình lai.}
...
\textbf{Bảng 3.14. Kết quả 5-fold cross validation của các mô hình học sâu.}
...
\textbf{Bảng 3.15. Kết quả ROC-AUC (One-vs-Rest) của tất cả các mô hình.}
...
\section{3.4. So sánh hiệu năng giữa các mô hình}
...
\subsection{3.4.1. So sánh hiệu năng phân loại tổng thể}
...
\textbf{Bảng 3.16. So sánh toàn diện hiệu năng năm mô hình trên tập kiểm thử.}
...
\textbf{Bảng 3.17. Tổng hợp kết quả 5-fold cross validation.}
...
\subsection{3.4.2. So sánh hiệu năng theo từng lớp cảm xúc}
...
\textbf{Bảng 3.18. F1-score theo từng lớp cảm xúc của năm mô hình.}
...
\subsection{3.4.3. So sánh chi phí tính toán và thời gian xử lý}
...
\textbf{Bảng 3.19. So sánh chi phí tính toán của các mô hình.}
...
\subsection{3.4.4. So sánh với các nghiên cứu liên quan}
...
\textbf{Bảng 3.20. So sánh kết quả với các nghiên cứu liên quan.}
...
\subsection{3.4.5. Bảng xếp hạng và đề xuất lựa chọn mô hình}
...
tốc độ suy luận | Đề xuất ứng dụng |
...
\textbf{Bảng 3.21. Xếp hạng và đề xuất ứng dụng các mô hình.}
...
\section{3.5. Phân tích và thảo luận kết quả}
...
\subsection{3.5.1. Phân tích cơ chế hiệu năng giữa các nhóm mô hình}
...
\subsection{3.5.2. Phân tích chuyên sâu các trường hợp dự đoán sai}
...
\textbf{Bảng 3.22. Phân bố nguyên nhân dự đoán sai trên 500 mẫu.}
...
\subsection{3.5.3. Đánh giá tác động của từng bước tiền xử lý (Ablation Study)}
...
\textbf{Bảng 3.23. Tác động của từng bước tiền xử lý lên hiệu năng mô hình PhoBERT-CNN-LSTM.}
...
\subsection{3.5.4. Phân tích hiệu năng theo lĩnh vực dữ liệu}
...
\textbf{Bảng 3.24. Hiệu năng PhoBERT-CNN-LSTM theo lĩnh vực dữ liệu.}
...
\subsection{3.5.5. Thảo luận tổng quan}
...
\section{3.6. Hạn chế của nghiên cứu}
...
\subsection{3.6.1. Hạn chế về dữ liệu}
...
\subsection{3.6.2. Hạn chế về mô hình}
...
\subsection{3.6.3. Hạn chế về xử lý ngôn ngữ}
...
\subsection{3.6.4. Hạn chế về đánh giá và triển khai}
...
\section{3.7. Hướng phát triển trong tương lai}
...
\subsection{3.7.1. Mở rộng dữ liệu và phạm vi nghiên cứu}
...
\subsection{3.7.2. Cải tiến mô hình và phương pháp}
...
\subsection{3.7.3. Giải quyết các thách thức ngôn ngữ}
...
\subsection{3.7.4. Triển khai ứng dụng thực tế}
